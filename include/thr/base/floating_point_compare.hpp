...
namespace thr {
    ...
    namespace details {
        ...
        struct float_wrapper {
            ...
            static constexpr value_type default_epsilon =
                static_cast<value_type>(5e-3); ///< 默认浮点比较误差阈值。
            value_type epsilon;                ///< 浮点比较误差阈值。

            // 构造、赋值、析构。
            /**
             * @brief 构造一个浮点包装类。
             * @param [in] rhs 浮点数值。
             * @param [in] eps 浮点比较误差阈值。
             */
            constexpr explicit float_wrapper(value_type rhs, value_type eps = default_epsilon) noexcept
                : value(rhs), epsilon(eps) {}
    ...
    struct no_nan_inf : public details::float_wrapper<T> {
      ...
      public:
         ...
         */
        constexpr explicit no_nan_inf(value_type rhs, value_type eps = default_epsilon) noexcept
            : base(rhs, eps) {
    ...
    constexpr bool operator==(no_nan_inf<T1> lhs, no_nan_inf<T2> rhs) {
        return std::abs(lhs.value - rhs.value) < std::min(lhs.epsilon, rhs.epsilon);
    }
