/**
 * @file maze_serialization_system.cpp
...
namespace thr::ecs {
    ...
    nlohmann::json maze_serialization_system::serialize_to_json(const entt::registry &registry) {
        nlohmann::json json;
        ...
        json["line_strips"] = {{"color",
                                [](sf::Color color) {
                                    ...
                                    throw std::invalid_argument(
                                        "in thr::ecs::maze_serialization_system::serialize_to_json(): "
    ...
    void maze_serialization_system::deserialize_from_json(entt::registry       &registry,
        ...
        if (auto line_strips_it = json.find("line_strips"); line_strips_it != json.end()) {
            ...
            line_strips.width = json_line_strips.value("width", 5.f);
        ...
        if (auto segments_it = json.find("segments"); segments_it != json.end()) {
            std::vector<entt::entity> segment_entities(segments_it->size());
            registry.create(segment_entities.begin(), segment_entities.end());

            for (const auto &[json_seg, entity] : std::views::zip(*segments_it, segment_entities)) {
                segment seg;
                ...
                if (!prev.is_null()) {
                    ...
                    node node{.position = seg.start_center};
                    registry.emplace<struct node>(entity, node);
                }
